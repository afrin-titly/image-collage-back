...
class HomeController < ApplicationController
  ...
  def home
    ...
    params[:images].each_with_index do |image, index|
      ...
      file = File.open("/tmp/storage/#{filename}")
      ...
      @images.push(image: ImageList.new("/tmp/storage/#{filename}"))
      File.delete("/tmp/storage/#{filename}")
    end
    color = params[:color]
    ...
    read_bg.write("/tmp/storage/collage.jpeg")
    blob = ActiveStorage::Blob.create_and_upload!(io: File.open("/tmp/storage/collage.jpeg"), filename: "collage.jpeg")
    File.delete("/tmp/storage/collage.jpeg")
